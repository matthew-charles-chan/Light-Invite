...
const getUsersInfoWithEventId = function (eventId, db) {
  return db.query(`
...
const getStartEnd = function (userId, db) {
  ...
  to_char(a.start_time + (select duration from events where events.id = a.event_id) * Interval '1 minute', 'HH24:MI') as end_time,
...
const forceStartEndWithDateId = function (dateId, db) {
  return db.query(`
  SELECT dates.id,
      to_char(start_time, 'Mon DD YYYY  HH24:MI') as start_time,
      to_char(start_time + (select duration from events where events.id = dates.event_id) * Interval '1 minute', 'HH24:MI') as end_time,
      ...
      JOIN events on events.id = event_id
      where dates.id = $1
      GROUP BY dates.id, title, events.description;`, [dateId])
  .then(response => forcePickedDateObject(response))
}

const forcePickedDateObject = function(dateArray) {
  output= []
...
const getUserByUserId = function(userId, db) {
  return db.query(`
    SELECT name, email
    FROM users
    WHERE users.id = $1
...
const updateEmailByUserId = function(email, id, db) {
  return db.query(`
  UPDATE users
  SET email  = $1
  WHERE id = $2
...
const getFirstDate = function (eventId, db) {
  return db.query(`
  SELECT dates.id
  FROM DATES
...
const forcePickDate = function (userId, db) {
  return getEventIdWithUserId(userId, db)
  .then (res => getFirstDate(res, db))
  .then (result => forceStartEndWithDateId(result, db))
}

const pickDate = function (userId, db) {
  return getVoteCount(userId, db)
  .then(async res => {
    let voteArray = res;
    let maxYes = 0;
...
module.exports =  {forceStartEndWithDateId, forcePickDate, addEvent, addDate, addUserGuest, getIdFromEmail, getStartEnd, pickDate, updateNameByUserId, makeAvailable, notAvailable, DeleteVote, creatorId, getVoteCount, getEventIdWithUserId, checkIfVoted, getUsersInfoWithEventId, getUserByUserId, updateEmailByUserId};
