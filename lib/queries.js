...
const getDatesWithEventId = function(eventId) {
  return dbParams.query(`SELECT id, start_time, yes_count, no_count
  FROM dates
  WHERE event_id = $1
...
const addEvent = function (event) {
  return dbParams.query(`INSERT INTO events (title, description, duration) VALUES ($1, $2, $3) RETURNING *`,
  [ event.title,
    event.description,
    event.duration
  ])
...
const addUserCreator = function (user) {
  return dbParams.query(`INSERT INTO users (event_id, name, email) VALUES ($1, $2, $3) RETURNING *`,
  [ user.event_id,
    user.email,
    user.email
  ])
