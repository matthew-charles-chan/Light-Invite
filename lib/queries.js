...
const getStartEndWithDateId = function (dateId, db) {
  ...
  SELECT date_id,
      ...
      where dates.id = $1
      GROUP BY date_id, dates.id;`, [dateId])
  .then(response => createDateObject(response))
  // .then(res => {
  //   return res;
  // })
...
module.exports =  { addEvent, addDate, addUserGuest, getIdFromEmail, getStartEnd, pickDate, updateNameByUserId, makeAvailable, notAvailable, DeleteVote, creatorId, getVoteCount, getEventIdWithUserId, checkIfVoted};
