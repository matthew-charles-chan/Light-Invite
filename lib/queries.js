...
const pickDate = function (eventId, db) {
  getVoteCount(eventId, db)
  .then(res => {
    let voteArray = res;
    let maxYes = 0;
    let maxDifferential = 0;
    let pick;
    for (const vote of voteObject) {
      let noCount = no_count;
      let yesCount = yes_count;
...
module.exports =  { addEvent, addDate, addUserGuest, getIdFromEmail, getStartEnd, pickDate, updateNameByUserId, makeAvailable, notAvailable, DeleteVote, creatorId, getVoteCount};
